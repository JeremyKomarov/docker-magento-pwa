...
if [ -f "$projectEnv" ]; then
  echo "The .env file already exists in the ./app/ directory."
else
  echo "Creating the .env file in the ./app/ directory..."
  cat << EOF > "$projectEnv"
...
#
#   Generated by @magento/pwa-buildpack v11.1.0 on 2022-02-14T12:19:11.117Z.
#
################################################################################
...
MAGENTO_BACKEND_URL=https://$HOSTS_URL/
#
################################################################################
...
EOF
  ...
  echo "The .env file has been created in the ./app/ directory."
fi
